...
| `GET /api/stats?days=7&pj=&dry=` | per-step consumption statistics (`total` / `by_model` / `by_step` / `by_day` / `by_pj` / `top`) |
...
| `stats` | per-step consumption statistics (the same aggregation as the Stats page; `days` / `pj` / `dry`) |
