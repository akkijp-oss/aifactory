...
- Navigation is ordered by how often each screen is used (board / file / runs / jobs / sandbox / keys / logs / settings). Press ++g++ then a letter (++b++ board, ++i++ file, ++r++ runs, ++j++ jobs, ++s++ sandbox, ++k++ keys, ++l++ logs, ++c++ settings) to jump; ++question++ lists the shortcuts
