...
flowchart LR
  ...
  B --> P[Manager<br>now, next and why, decision log]
...
| Manager | One page for what the manager (PM) is doing now, what it intends to run next, and why (it only reads `GET /api/pm`, every five seconds). From the top: *Now* (the state and the run it is based on), *Next* (the next ticket and the reason), *Decisions* (newest first: time, subject, what was decided and **why**), and *Board summary* (todo, in progress, review, waiting for a human, done, plus the tickets waiting for a human). **When the board cannot be read yet it does not say "there is no ticket to run"; it says the state could not be read** (a freshly installed console is normally in this state, so it is not shown as a danger). Counts that cannot be read are never written as zero. The decision log starts filling up once the manager makes decisions | Read only. Only the project filter does anything. *Switch mode*, *Pause* and *Run now* are placeholders: the page says they are not wired up yet, and pressing one repeats that |
...
- Navigation is ordered by how often each screen is used (board / manager / file / runs / jobs / sandbox / stats / keys / logs / settings). Press ++g++ then a letter (++b++ board, ++p++ manager, ++i++ file, ++r++ runs, ++j++ jobs, ++s++ sandbox, ++t++ stats, ++k++ keys, ++l++ logs, ++c++ settings) to jump; ++question++ lists the shortcuts
...
| `pm_status` | The manager's (PM's) state; read-only and starts nothing (ADR-0074). Same content as `GET /api/pm` (both call `core.pm_status()`). `proposal` carries the next move |
| `pm_tick` | Runs one turn of the manager's loop (`core.pm_tick()`). **This version only proposes; it starts no run.** Same content as `POST /api/pm/tick`. Use it to take a turn right now instead of waiting for the timer (`aifactory-pm.timer`) |
