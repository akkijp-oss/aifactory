...
| `GET /api/pm[?pj=]` | The manager's (PM's) state (`core.pm_status()`; read-only — it starts nothing. ADR-0074). `state` is one of `idle` / `waiting` / `landing` / `blocked`, derived every time from the existing records rather than stored. **"could not read it" and "zero of them" are different values**: the board carries `board.readable` and `board.reason` (`ok` / `no_db` / `kb_failed`; `counts` is `null` when the board could not be read), and the run records carry `runs.readable` and `runs.reason` (`ok` / `no_records` / `error`). `next` is always an object, and `next.reason` is one of `picked_next` / `no_todo` / `run_running` / `landing_observed` / `needs_human` / `board_unreadable`. `next.launchable` is true only for `picked_next`, and a `null` `next.ticket` does not mean "all good" |
