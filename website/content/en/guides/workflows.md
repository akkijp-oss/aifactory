...
|---|---|---|---|
| **hotfix** | plan → implement → gates → review → pr | Minimal fix for a production incident. Targets `hotfix_base` (usually `main`) | PR → human |
| **bug** | plan → implement (reproduction test first) → gates → review → pr | Bug fixes | PR → human |
| **feature** | research → design → implement → gates → review → pr | New functionality, with research and design steps | PR → human |
| **chore** | implement → gates → pr | Docs, dependency updates, settings; no judgement needed. No plan, no review | PR → human |
...
Boxes with a model name are agent steps (`claude -p` inside the VM); `code` boxes are code steps (Mac-side scripts executed on the VM). Dotted lines are send-backs; past the limit the run goes to `human`.
...
Adding one file `workflow/kit/workflows/<name>.yml` makes it available as a `kind` in `kb new`. The format is described in [workflow yml](../reference/workflow-yml.md). Steps use the four existing roles or the code steps (`gates.sh` / `pr-create.sh` / `pr-merge.sh`); a new role needs `workflow/kit/roles/<role>.md`.
