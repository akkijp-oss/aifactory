...
The distribution source is the `main` branch. The installer downloads source from the same ref and builds with an official Go toolchain verified against its SHA-256 manifest. No preinstalled Go or manually copied worker binaries are required. Initial downloads, especially a Mac VM image, may take time.

## Prepare the control plane

Run a control plane at v0.3.0 or later, which serves `POST /v1/check`, and enroll each worker separately:
...
| `AIFACTORY_REF` | Source Git ref, default `main`. To pin a version, use a commit SHA here and in the entry-script URL |
