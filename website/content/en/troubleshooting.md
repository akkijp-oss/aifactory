...
| A run stops with a skewed guest clock / commit and ADR dates are days old | `clock_offset_s` in the run record, the `[clock]` line in the `sandbox take` log | A rolled-back VM restarts its clock from the moment the snapshot was taken ([ADR-0069](decisions/index.md)). A run stopped this way has already returned its VM, so just rerun it with `kb reopen` → `kb run` (the next lease re-syncs the clock). If a VM is still leased and its date is off, `sandbox reset <task>` lends it again. If that does not help, run `sudo systemctl restart systemd-timesyncd` inside the VM, then check that NTP (udp/123) can leave the network |
...
| Symptom | Look at | Fix |
|---|---|---|
| Authentication error (`failure: key`) | Tail of `agent-<step>-<n>.log` and `key=… (pool: <name>)` in the run log | On the control plane: `claude setup-token` → `sandbox keys token <name>` (or the console's *Keys* screen). Intake's key: `sandbox token rotate claude`. Continue the stopped run with `kb run <id> --from` |
