...
|---|---|---|---|
| Claude Code long-lived token (`claude setup-token`) | Mac `~/.config/sandbox/pj/<pj>.env`, per project (ADR-0006) | Into `/run/sandbox/env` (tmpfs) on every take. `reinject` swaps it while lent | Gone on rollback. The token's own expiry is set by Anthropic |
...
None of them are written into the repository. `.gitignore` excludes `*.env` / `*.token` / `.env*`. `sandbox token show` masks values.
...
| Tokens per project (ADR-0006) | A leak in one project does not reach the others. Each can be rotated independently |
