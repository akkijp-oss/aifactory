...
aifactory の導入に必要な機材、アカウント、ソフトウェアをまとめています。それぞれの用途を確認し、セットアップを始める前に準備してください。
...
| もの | 要件 | 何のため |
|---|---|---|
| Proxmox VE ホスト | 9.x、LVM-thin のローカルストレージ。RAM は「VM 1 台 8 GB × 台数 + 余裕」（プロジェクト 3 つ × 3 台 + 汎用 3 台なら 100 GB 程度） | VM プールを置く。ホスト名は `~/.config/sandbox/env` の `PVE_HOST`（ssh エイリアス）で指す |
...
工場に入れるリポジトリ（プロジェクト）は、`$AIFACTORY_WORKSPACE/projects/<pj>/` に 3 ファイル（`provision.sh` / `project.yml` / `gates.sh`）を置いて定義します。同梱の例が 1 つあります。

| プロジェクト | リポジトリ | スタック | 置き場 |
|---|---|---|---|
...
新しいプロジェクトの登録手順は [プロジェクトを追加する](../guides/add-project.md) を参照してください。
...
4. **プロジェクト定義**: `$AIFACTORY_WORKSPACE/projects/<pj>/`（provision.sh / project.yml / gates.sh）

この 4 点以外（kanban / glue / ワークフロー）は環境に依存しません。
