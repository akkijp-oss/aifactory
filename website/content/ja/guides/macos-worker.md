...
- **ゲストが生きていて、leaseも保持している** → 5へ進まない。leaseを解放しないまま `kb run <id> --resume` で続ける。再開できるのは、認証情報・リポジトリ・工程履歴がまだ無いprovision失敗のときに限る（「失敗時の復旧」）。
