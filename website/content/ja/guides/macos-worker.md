...
**5. 再開する。** leaseを片づけたら `kb run <id> --resume` で続ける。どの工程から再開するか、再開できない条件、`--resume` がleaseの所有を要求することは「失敗時の復旧」と [ADR-0047](https://github.com/akkijp-oss/aifactory/blob/main/docs/adr/0047-resume-start-step-from-history.md) にある。この節では繰り返さない。
