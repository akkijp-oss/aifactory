...
「鍵」画面の上の**残量（利用枠）**で鍵を選ぶと、5 時間枠 / 7 日枠（全体）/ 7 日枠（Fable）の現在値と回復までの時間を確認できます。3 枠の推移は一つのグラフに重なり、既定で過去 7 日と未来 7 日を表示します。実測は実線、予測は破線です。凡例を押して線を切り替え、グラフをなぞるか左右キーを使うと時点ごとの残量を読めます。

予測は窓の開始からの平均消費ペースが続く場合の目安です。回復時刻で 100% に戻り、その後も同じ間隔で回復すると仮定します。観測から 15 分を超えた枠、回復時刻不明・経過済みの枠、開始から 5% 未満しか経っていない枠は予測を出しません。実測のない期間は埋めません。

制御系の timer `aifactory-keys-probe.timer` は 5 分ごとに、使わない設定や Fable に割り当てない設定も含め、全鍵に Fable で問い合わせます。共通枠を取得できなければ安いモデルでも確認します。取得できない Fable 枠は理由と前回の値を表示し、0% と決めつけません。問い合わせは少量の利用枠を消費します。配車の設定は変わりません。`--cheap` を明示した場合だけ共通枠の確認に限定します。旧 `AIFACTORY_KEYS_PROBE_FULL_INTERVAL_S` 設定は使いません。
