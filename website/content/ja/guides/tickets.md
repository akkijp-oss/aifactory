...
- **手元の CLI**: `console/bin/attach <チケット番号> <ファイル>...` を**あなたの端末で**実行すると、ファイルをそのままコンソールの API へ送って添付します。中身も base64 も AI セッションの文脈やログには出ません。詳しくは [Web コンソール](console.md#attach-local-file)
- **intake**: `glue/bin/intake memo.txt --attach 画面.png` のように渡すと、起票したチケットに添付します。画像は intake の LLM にも見せるので、本文の `## 現状` に画面から読み取れた事実が入ります

!!! note "名前は整えられます"
    ファイル名は保存するときに整えられます（パス区切り・`..`・制御文字・Markdown の記法（`` ` `` `*` `[` `]` `<` `>` `|`）を落とし、120 バイトに切ります）。名前は各工程の依頼文にそのまま埋まるためです。実際に保存された名前は `kb attachments <id>` とチケット画面で確かめられます。

!!! warning "起票と添付は別の操作です"
    `kb new --attach` と起票画面の「起票する」は、**チケットは作れたのに添付だけ失敗する**ことがあります（上限を超えたときなど）。そのときもチケットは残っているので、`kb attach <id> <ファイル>` かチケット画面の「添付する」でやり直してください。
