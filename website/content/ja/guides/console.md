...
| 鍵 | Claude の鍵プール（制御系の `keys.json`）の一覧。名前・「fable に使う」/「fable 以外に使う」のフラグ・使うかどうか・トークンの末尾 4 文字・発行日・最終利用・使用回数・その鍵を使っている貸出中のチケット。トークンの値はどこにも出ない | 鍵を足す（名前・トークン・フラグ・メモ。送ったあとトークンは画面に残らない）、フラグと「使う」の切り替え、値の差し替え、削除（危険色のダイアログ。使っているチケットがあれば名前の入力を求める）。使わない設定にする / 消すと、その鍵を使っている貸出に `sandbox reinject` のジョブが自動で起きる（動いている claude はそのままで、次の起動から別の鍵になる） |
