...
- ナビは使う頻度の順（ボード / 管理役 / 起票 / 実行記録 / ジョブ / sandbox / 統計 / 鍵 / ログ / 設定）です。++g++ に続けて頭文字（++b++ ボード、++p++ 管理役、++i++ 起票、++r++ 実行記録、++j++ ジョブ、++s++ sandbox、++t++ 統計、++k++ 鍵、++l++ ログ、++c++ 設定）で移動でき、++question++ で一覧が出ます
...
| `pm_status` | 管理役（PM）の状態を返します（読み取りだけで、何も起動しません。ADR-0074）。`GET /api/pm` と同じ内容です（どちらも `core.pm_status()` を呼びます）。`proposal` に「次の一手」が付きます |
| `pm_tick` | 管理役の 1 周を 1 回だけ回します（`core.pm_tick()`）。**この版は提案だけで、run は起こしません**。`POST /api/pm/tick` と同じ内容です。定期実行（`aifactory-pm.timer`）を待たずに今すぐ 1 周させたいときに使います |
