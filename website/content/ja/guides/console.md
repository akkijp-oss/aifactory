...
| `GET /api/next[?pj=]` | 「配車する」を押す前の下見。**配車（`dispatch`）と同じ判定**（`core.pm_pick_next`）で選び直した票を返します（ADR-0079）。`next`（回せる票。無ければ `null`）/ `reason`（AI Factory Manager と同じ語彙。`picked_next` / `no_todo` / `blocked_by_dependency` / `blocked_by_pause` / `blocked_by_key`）/ `kb_next`（薄い `kb next` が返した生の番号）/ `skipped_by_dependency`（飛ばした票 → 未完了の先行票）/ `skipped_by_pause`（飛ばした票 → 一時停止の事実。`paused` は `quota` / `nokey`、`until` は解除時刻、`hits_exceeded` は利用枠切れが続いた回数の超過）。飛ばした票と理由は配車のダイアログに 1 行ずつ出ます。**配車のときの環境で決まるもの**（PJ の project.yml・Pull worker・プールの空き）はここでは確かめません（確かめていないことは画面でもそう書いています） |
| `GET /api/pm[?pj=]` | AI Factory Manager（PM）の状態（`core.pm_status()`。読み取りだけで、何も起動しません。ADR-0074）。`state` は `idle` / `waiting` / `landing` / `blocked` の 4 つで、保存せず既存の記録から毎回導きます。**「取得できていない」と「0 件」は別の値です**: 板は `board.readable` と `board.reason`（`ok` / `no_db` / `kb_failed`）、run の記録は `runs.readable` と `runs.reason`（`ok` / `no_records` / `error`）。`counts` は板の集計を読めたときだけ入ります（読めなければ `null`。`kb_failed` には集計が読めた場合と読めなかった場合の両方があるので、`counts` の有無を `board.readable` の代わりに使わないでください。板を読めたかは `board.readable` / `board.reason` を見ます）。`next` は常に object で、`next.reason` は `picked_next` / `no_todo` / `run_running` / `landing_observed` / `needs_human` / `board_unreadable` / `blocked_by_dependency` / `blocked_by_pause` / `blocked_by_key`。`next.reason` は板を読めたかを先に見るので、`waiting` / `landing` でも板が読めていなければ `board_unreadable` になります。`next.launchable` が真なのは `picked_next` のときだけで、`next.ticket` が `null` でも「順調」という意味にはなりません。`pj` を省くと全 PJ 横断になり、どれか 1 つでも走っていれば `waiting`、直近の止まった run が 1 本詰まっていれば `blocked` になります。`proposal` に「次の一手」が付きます（`core.pm_decide()`。副作用はありません）: `{pj, ticket, run, state, action, reason_code, facts, mode}` で、`action` は `none` / `run` / `requeue`、`reason_code` は判断の記録と同じ語彙（ADR-0074 決定 5）だけを使います。**`proposal` が `null` なのは「することがない」ではなく「板か実行記録を読めていない」という意味です**。止まった run をもう一度回せると AI Factory Manager が判断したときは、`state` が `blocked` ではなく `idle` になり、`next.reason` は `requeue_proposed`（`picked_next` とも `no_todo` とも別の値）で、`next.launchable` は偽のままです（提案であって起動の許可ではありません）。チケットが[先行条件](../reference/cli-kb.md)（`depends_on`）を宣言していて、その先行票がまだ終わっていなければ、AI Factory Manager はその票を飛ばして次の todo を選びます。候補が全部そうなら `next.reason` と `proposal.reason_code` は `blocked_by_dependency` になり、`next.ticket` は `null`、`state` は `idle` のままです（板は読めていて、**確かめた結論**として選べないので、`no_todo` とも `board_unreadable` とも別の値です）。飛ばした票と、その未完了の先行票は `proposal.facts.skipped_by_dependency`（`{"538": {"537": "blocked"}}` の形。DB に無い番号は `null`）に残ります（ADR-0077）。鍵の利用枠切れや鍵待ちで一時停止中のチケット（[`kb resumable`](../reference/cli-kb.md)）も、解除されるまでは同じように飛ばします。候補が全部そうなら、**誰が解けるかで理由が 2 つに分かれます**（ADR-0081）。利用枠切れ（`paused: quota`・`until` に解除時刻）だけなら `blocked_by_pause` で、`state` は `idle` のままです——解除時刻が来れば `dispatch --resume-paused` の timer が続きを回すので、人の出番はありません。鍵待ち（`paused: nokey`・`until` は `null`。**鍵プールに鍵が登録されるまで解けません**。時間では変わりません）か、利用枠切れが続いた票（`hits_exceeded`。**人が枠を確かめるまで自動では回しません**）が 1 件でも混ざっていれば `blocked_by_key` で、**`state` は `blocked`（人の確認待ち）に上がります**——自動再開の待ち行列に入らない票なので、待っていても進みません。どちらの場合も飛ばした票は `proposal.facts.skipped_by_pause` に残るので、`paused` / `until` / `needed_keys` / `hits_exceeded` を見れば「どの票が・何を待っているか」が分かります（AI Factory Manager の画面にも 1 票 1 行で出ます） |
