...
| チケット | 本文（Markdown）、状態の履歴、関連する run とジョブ | `kb run`（ダイアログで PJ・ワークフロー・所要を確かめてから。dry-run / `--keep` / `--resume`）、状態を進める（開始 / レビュー待ち / 完了 / 未着手に戻す / 人間待ち。確認は出ず、トーストの「元に戻す」で戻せる）、種別・PR 番号・メモの修正、run 記録からの状態同期（押す前に対象の run と前後の状態・メモが出る。run の後にチケットが更新されていれば警告つき） |
