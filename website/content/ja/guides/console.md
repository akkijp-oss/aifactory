...
| 設定 | ワークフローの一覧（名前・うまくいったときの流れ・うまくいかなかったときだけ回る工程）、モデルの経路（`routes.env`。4 行それぞれに入力欄があり、ここから直せる。モデルは Agent → モデル名の 2 段で表示名から選ぶか、ID を直接入力する）、`git status`。workflow の名前と工程は本物のリンクで、開くと工程の詳細（担い手・指示・読み書き・上限・分岐・実効モデル）が読める | workflow を開く、工程を開く、定義の原文を読む、モデルの経路を変える（影響する工程を見てから保存する） |
...
開いただけではジョブ・runner・設定は動きません。変えられるのは「モデルの経路」（トップの 4 行）と workflow → 工程から開く「使用モデル」だけで、どちらも影響する工程を見てから保存します（下の「モデルを変える」）。
