...
flowchart LR
  ...
  B --> P[AI Factory Manager<br>いま・次とその理由・判断の記録]
...
| AI Factory Manager | AI Factory Manager（PM）がいま何をしていて、次に何を回すつもりで、それはなぜかを 1 枚にまとめた画面（`GET /api/pm` を 5 秒ごとに読むだけ）。上から「いま」（状態と、根拠にしている run）、「次に回すもの」（次のチケットとその理由）、「判断の記録」（新しい順に、日時・対象・決めたこと・**理由**）、「板の要約」（未着手・実行中・レビュー待ち・人間待ち・完了の件数と、人の確認を待っているチケット）の順。**まだ板を読めていないときは「回すチケットがありません」とは言わず、「状態を取得できていません」と出ます**（置き場を作ったばかりの console は正常にこの状態になるので、危険色にはしません）。件数を読めないときも 0 件とは書きません。判断の記録は AI Factory Manager が判断してから並び始めます | 読むだけです。PJ の絞り込みだけが効きます。「モードを切り替える」「一時停止する」「今すぐ回す」は置き場だけで、まだつながっていないことを画面に書いてあります（押すと同じ説明が出ます） |
...
| 鍵 | Claude のトークン（`claude setup-token` の長期トークン）を「鍵」として複数登録しておく画面（制御系の `keys.json`）。名前とメモ・「Fable に使う」/「Opus・Sonnet に使う」のチェック・有効かどうか・トークンの末尾 4 文字・登録日・最後に起動した日時・起動回数（runner が実際にその鍵で `claude` を起動した回数。割り当て回数はマウスを乗せると出る）・使用中のチケット。トークンの値はどこにも出ない。表の下に、鍵の選び方（モデルごとに、有効でチェックの合う鍵のうち最後に使ってから一番時間が経った鍵）・実行中は途中で変わらないこと・合う鍵が無い run は一時停止し、登録すると自動で回し直すこと・有効を外したときの動きが書いてある 表の上に**残量（利用枠）**があり、鍵ごとに 5 時間枠 / 7 日枠（全体）/ 7 日枠（Fable）の残り %・リセットまでの時間・窓の始点 → 終点・「このペースだと約 n で枯渇」などの一言を出す（残り 10% 以下は赤、25% 以下は橙）。観測は制御系の timer（`aifactory-keys-probe.timer`。有効・用途設定を問わず全鍵に 5 分ごとに Fable で問い合わせる。共通枠が欠ければ安いモデルでも確認する）で、最後に残量を読めた観測が 15 分より古ければその旨が出る（叩き続けていても読めない回が続けば「古い」と出る。表示している残量が最後に読めたときの値だから）。鍵切れ（認証が通らない）は run が止まる前にここに出る。一覧の「残量」列はいちばん逼迫している窓。上部の**残量の推移と予測**は、選んだ鍵の 3 枠を一つのグラフに表示する（既定は過去 7 日・未来 7 日。実測は実線、予測は破線。データ不足や古い観測では予測を出さない）（ADR-0093） | 鍵を登録する（名前・メモ・トークン・どのモデルに使うか。送ったあとトークンは画面に残らない）、チェックと「有効」の切り替え、トークンの入れ替え、削除（危険色のダイアログ。使用中のチケットがあれば名前の入力を求める）。有効を外す / 消すと、その鍵を使っている実行中の VM に別の鍵を入れ直すジョブ（`sandbox reinject`）が自動で起きる（動いている工程はそのまま終わり、次の工程から別の鍵に切り替わる）。「いま調べる」でその場で観測する（ジョブ。実行中なら二重に起こさない） |
...
設定の workflow の一覧は、名前も工程も本物のリンクです。Tab で届き Enter で開き、パンくずで一覧へ戻れます。

一覧の「流れ」の列は **うまくいったときの流れ** です。`feature` の `resolve` のように、ほかの工程が
うまくいかなかったときだけ回る工程は、その下に別立てで並びます（定義の順に一列に並べると、常に回るように読めるためです）。

workflow を開くと、説明・うまくいったときの流れ・うまくいかなかったときだけ回る工程・工程の一覧・
定義（ブランチの作り方、PR の宛先、workflow の入力、yml の置き場）が出ます。「定義の原文を読む」で yml そのものも読めます。

工程を開くと、次のものが読めます。

- **担い手**: 役割（`planner` / `implementer` / `reviewer` / `researcher`）か、機械（`code` の script）か
- **この workflow での指示**（yml の `brief`）と、読むファイル・書くファイル（`inputs` / `outputs`）
- **時間の上限**（`timeout_min`。定義に書いていなければ「定義に無いので既定」と添えます）
- **分岐**: うまくいったとき・いかなかったときの行き先。戻す工程には最大何回かと、超えたときの行き先が付きます。
  レビューの工程には、指摘が軽微なときに戻せる回数が 1 回だけ増えることがある（ADR-0053）という注記が付きます
- **モデル**: クラス（役割の既定か、その工程の `model_class` か）→ 経路（`routes.env` の `MODEL_<クラス>`）→ モデル名の順。
  `code` の工程は「機械が実行します。モデルは使いません。」と出て、モデル名は出ません

モデルは **設定から解けるところまで** しか出しません。run を起こすときに環境変数 `CLAUDE_MODEL` で上書きできるので、
実際に使われたモデルは設定からは分かりません。実績は「統計」の画面（モデル別・工程別）で見てください。

定義が読めない workflow、schema に無いキー、`role` の綴り違いも隠さず「読めない項目」として並びます。
開いただけではジョブ・runner・設定は動きません。変えられるのは「モデルの経路」（トップの 4 行）と、workflow → 工程から開く
「使用モデル」および「この工程の定義」で、どれも影響する工程を見てから保存します（下の「モデルを変える」）。
どの欄も Agent（実行する CLI。今は claude だけ）→ モデル名（表示名）の 2 段で選べ、表に無いモデルは ID を直接入力できます（ADR-0072）。

「この工程の定義」の欄には、**その工程の yml ブロック（`- id: <工程>` の行から次の工程の直前まで）だけ**が原文のまま出ます。
`timeout_min` や分岐のようにモデル以外の項目もここから変えられ、モデルの欄とは画面の中で双方向に同期します。
ファイル全体は出ませんし書けません。工程の `id` と担い手の種類（`role` / `code`）は変えられません。
読めない書き方のときは 1 バイトも書かずに理由が出て、入力は消えません（ADR-0073）。
...
| `GET /api/next[?pj=]` | 「配車する」を押す前の下見。**配車（`dispatch`）と同じ判定**（`core.pm_pick_next`）で選び直した票を返します（ADR-0079）。`next`（回せる票。無ければ `null`）/ `reason`（AI Factory Manager と同じ語彙。`picked_next` / `no_todo` / `blocked_by_dependency` / `blocked_by_pause`）/ `kb_next`（薄い `kb next` が返した生の番号）/ `skipped_by_dependency`（飛ばした票 → 未完了の先行票）/ `skipped_by_pause`（飛ばした票 → 一時停止の事実。`paused` は `quota` / `nokey`、`until` は解除時刻、`hits_exceeded` は利用枠切れが続いた回数の超過）。飛ばした票と理由は配車のダイアログに 1 行ずつ出ます。**配車のときの環境で決まるもの**（PJ の project.yml・Pull worker・プールの空き）はここでは確かめません（確かめていないことは画面でもそう書いています） |
| `GET /api/pm[?pj=]` | AI Factory Manager（PM）の状態（`core.pm_status()`。読み取りだけで、何も起動しません。ADR-0074）。`state` は `idle` / `waiting` / `landing` / `blocked` の 4 つで、保存せず既存の記録から毎回導きます。**「取得できていない」と「0 件」は別の値です**: 板は `board.readable` と `board.reason`（`ok` / `no_db` / `kb_failed`）、run の記録は `runs.readable` と `runs.reason`（`ok` / `no_records` / `error`）。`counts` は板の集計を読めたときだけ入ります（読めなければ `null`。`kb_failed` には集計が読めた場合と読めなかった場合の両方があるので、`counts` の有無を `board.readable` の代わりに使わないでください。板を読めたかは `board.readable` / `board.reason` を見ます）。`next` は常に object で、`next.reason` は `picked_next` / `no_todo` / `run_running` / `landing_observed` / `needs_human` / `board_unreadable` / `blocked_by_dependency` / `blocked_by_pause`。`next.reason` は板を読めたかを先に見るので、`waiting` / `landing` でも板が読めていなければ `board_unreadable` になります。`next.launchable` が真なのは `picked_next` のときだけで、`next.ticket` が `null` でも「順調」という意味にはなりません。`pj` を省くと全 PJ 横断になり、どれか 1 つでも走っていれば `waiting`、直近の止まった run が 1 本詰まっていれば `blocked` になります。`proposal` に「次の一手」が付きます（`core.pm_decide()`。副作用はありません）: `{pj, ticket, run, state, action, reason_code, facts, mode}` で、`action` は `none` / `run` / `requeue`、`reason_code` は判断の記録と同じ語彙（ADR-0074 決定 5）だけを使います。**`proposal` が `null` なのは「することがない」ではなく「板か実行記録を読めていない」という意味です**。止まった run をもう一度回せると AI Factory Manager が判断したときは、`state` が `blocked` ではなく `idle` になり、`next.reason` は `requeue_proposed`（`picked_next` とも `no_todo` とも別の値）で、`next.launchable` は偽のままです（提案であって起動の許可ではありません）。チケットが[先行条件](../reference/cli-kb.md)（`depends_on`）を宣言していて、その先行票がまだ終わっていなければ、AI Factory Manager はその票を飛ばして次の todo を選びます。候補が全部そうなら `next.reason` と `proposal.reason_code` は `blocked_by_dependency` になり、`next.ticket` は `null`、`state` は `idle` のままです（板は読めていて、**確かめた結論**として選べないので、`no_todo` とも `board_unreadable` とも別の値です）。飛ばした票と、その未完了の先行票は `proposal.facts.skipped_by_dependency`（`{"538": {"537": "blocked"}}` の形。DB に無い番号は `null`）に残ります（ADR-0077）。鍵の利用枠切れや鍵待ちで一時停止中のチケット（[`kb resumable`](../reference/cli-kb.md)）も、解除されるまでは同じように飛ばします。候補が全部そうなら `blocked_by_pause` で、飛ばした票は `proposal.facts.skipped_by_pause` に残ります（ADR-0079）。**この状態は `blocked` に格上げしませんが、「放っておけば機械が片付ける」という意味ではありません**: 解け方が 3 つあり、`skipped_by_pause` の中身で見分けます。利用枠切れ（`paused: quota`・`until` に解除時刻）は時刻が来れば `dispatch --resume-paused` が続きを回します。鍵待ち（`paused: nokey`・`until` は `null`）は**鍵プールに鍵が登録されるまで解けません**（時間では変わりません）。利用枠切れが続いた票（`hits_exceeded`）は**人が枠を確かめるまで自動では回しません**。後ろの 2 つは自動再開の待ち行列にも入らないので、`blocked_by_pause` が続くときは `skipped_by_pause` の `paused` / `until` / `needed_keys` / `hits_exceeded` を見てください |
...
| `GET /api/keys/history?hours=168&name=` / `POST /api/keys/probe` | 残量の履歴と予測（`now` / `points` / `forecasts`。予測できない枠は `reason` を返します。`window_start` は合成行で実測として描きません）/ 残量をいま調べるジョブを起こす `{full}`（既定は Fable でも叩く） |
...
| `pm_status` | AI Factory Manager（PM）の状態を返します（読み取りだけで、何も起動しません。ADR-0074）。`GET /api/pm` と同じ内容です（どちらも `core.pm_status()` を呼びます）。`proposal` に「次の一手」が付きます |
| `pm_tick` | AI Factory Manager の 1 周を 1 回だけ回します（`core.pm_tick()`）。**この版は提案だけで、run は起こしません**。`POST /api/pm/tick` と同じ内容です。定期実行（`aifactory-pm.timer`）を待たずに今すぐ 1 周させたいときに使います |
