...
`tools/list` は全ツールに `annotations`（`title` / `readOnlyHint`、返却と停止には `destructiveHint`）を返します。これが無いと Claude Code は「並列に呼べないツール」とみなして同じターンの呼び出しを直列に送るので、サーバーが非同期でも待たされます（ADR-0036）。
