...
宣言を読むには pyyaml が要ります。入っていない・`project.yml` が読めない環境では、照合を行わず（警告も出さず）素通りします。起票の挙動は変わりません。
...
未完了（`done` 以外）の先行票を 1 つでも持つチケットは、**AI Factory Manager（PM）も配車（`dispatch`）も次に選びません**。PM はその票を飛ばして
...
- `kb next` は先行条件も一時停止も見ません（todo を番号順に 1 件返すだけの口です）。先行条件の判定は
  `console/lib/core.py` の 1 か所（`pm_unmet_deps`）にあり、[`dispatch`](cli-glue.md) はこれを直に、PM とコンソールの
