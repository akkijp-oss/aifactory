...
モデルは **Agent（実行する CLI。今は claude だけ）→ モデル名（`Opus 5` のような表示名）** の 2 段で選びます。選ぶと下の
モデル ID の欄に値が入り、保存されるのは今までどおりモデル ID 1 つです。表示名と ID の対応表の正本は
`console/lib/core.py` の `MODEL_CATALOG` で、この文書には写しません。表に無いモデルは ID の欄に直接入力でき、
設定に既に入っている表に無い値はそのまま選択肢に残ります（ADR-0072）。
...
!!! note "入れられるモデル名"
    ...
    画面からは保存できません（ファイルを直接編集すれば入ります）。モデル名は表示名の表から選びますが、
    表は「選べる候補」であって保存できる値の一覧ではありません。表に無い新しいモデル名も直接入力できます。

## 工程の定義そのものを変える

同じ画面（設定 → workflow → 工程）に「この工程の定義」の欄があり、**その工程の yml ブロックだけ**が原文のまま出ます。
`timeout_min` や分岐（`next` / `on_pass` / `on_fail`）、読み書きするファイル、`brief` のようにモデル以外の項目も、
ここから変えられます。

- 出るのも書けるのも **`- id: <工程>` の行から次の工程の直前まで**です。ファイル全体は出しませんし、書けません（ADR-0073）。
- 上のモデルの欄とは画面の中だけで同期します。yml を直すとモデルの欄に出て、モデルの欄を直すと yml の該当行が変わります
  （同期するのは `model` と `model_class` の 2 行だけです）。まだ保存はしていません。
- **工程の `id` と担い手の種類（`role` か `code` か）は変えられません。** ほかの工程の行き先がその `id` を指しているのに、
  参照が壊れていないかを確かめる仕組みが無いからです。
- 保存の流れはモデルを変えるときと同じです（下見 → 確認 → `base_sha256` で競合を見る → 控えを残す → 記録に 1 行）。
  読めない書き方のときは 1 バイトも書かず、理由が欄の下に出て、入力は消えません。
- 書かれるのはその工程のブロックだけです。ほかの工程・ファイルの先頭のコメント・項目の並び・引用は 1 文字も変わりません。
