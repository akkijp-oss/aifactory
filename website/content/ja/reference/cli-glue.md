...
- 通常の配車で票を選ぶ口は `kb list --status todo`（`--pj` があればそれも）を番号順に舐め、候補 1 件を `kb show <id>` で読む、の 2 つです（`--resume-paused` で続きを回す票だけは `kb resumable --json` から選びます。下の箇条書き）。`kb next` は呼びません（`kb next` はコンソールの下見 `GET /api/next` と PM の `pm_status` が使う口です）。ADR-0077 の「結果」節にある「`bin/dispatch`（`kb next` → `kb run`）」は事実誤認で、正しい経路は ADR-0078 の「状況」節にあります
- 直列。1 件終わるまで次は始めない
