...
| `--depends` | 先行票の番号をカンマ区切りで（例 `534,535`）。そのまま `kb new --depends` に渡す。人が渡した値だけで、本文の自由文からは依存を推測しない（詳しくは [kb](cli-kb.md) の「先行条件」） |
...
5. 本文末尾に `（intake <日時> / model <モデル> / confidence <値> / <理由>）` を付けて `kb new`。本文冒頭に `pr: N` があれば `--pr` に回す。`--attach` は `kb new --attach` に、`--depends` は `kb new --depends` にそのまま渡す。**チケットは作れたのに添付だけ失敗した**ときは、id を出してログも書いたうえで終了コード 2 で終わる（`kb attach <id> <ファイル>` でやり直す）
...
- 通常の配車で票を選ぶ口は `kb list --status todo`（`--pj` があればそれも）を番号順に舐め、候補 1 件を `kb show <id>` で読む、の 2 つです（`--resume-paused` で続きを回す票だけは `kb resumable --json` から選びます。下の箇条書き）。`kb next` は呼びません（`kb next` はコンソールの下見 `GET /api/next` と PM の `pm_status` が、最初の候補を得るために使う口です。どちらもその返りを `core.pm_pick_next` に通し、配車と同じ規則で選び直します = ADR-0079）。ADR-0077 の「結果」節にある「`bin/dispatch`（`kb next` → `kb run`）」は事実誤認で、正しい経路は ADR-0078 の「状況」節にあります
- 直列。1 件終わるまで次は始めない
