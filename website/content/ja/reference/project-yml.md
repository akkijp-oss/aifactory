...
### `capabilities`（この環境でできないこと）

実行環境に無い手段を要求する完了条件は、そのまま受理されると run が満たせないまま PR に着地します。`capabilities` はその環境に何があるかを宣言し、票の完了条件と機械で突き合わせるための項目です（ADR-0080）。

```yaml
capabilities:
  browser: false    # 画面のある実ブラウザで目視・操作できるか（ヘッドレスしか無いなら false）
  docker: false     # docker / compose でコンテナを起動できるか
  egress: true      # 外部ネットワークから取得できるか
  gui: false        # デスクトップ画面を操作できるか
```

4 つのキーはすべて任意です。**書かないキーは「未宣言」で、照合の対象になりません**。`false`（無いと確かめた）とは別の扱いなので、実測していない能力は書かないでください。`computer_use: true` のときは `gui` を書かなくても `true` として扱われます（`gui: false` と書くことはできません）。

宣言があると 3 か所が動きます。

- **起票**: `kb new` / `intake` / `ticket_new` が本文の `## 完了条件` を走査し、宣言と食い違う行を警告します。**起票は止まりません**（rc も変わらず、警告は stderr と `ticket_new` の `warnings[]` に出るだけ）。自由文が相手なので誤検知も取りこぼしも出ます。判定に使わず、人に見せるための材料です。
- **起動**: 依頼文に「## この環境で検証できないこと」節が入ります（全役割）。無い能力の一覧と、当たった完了条件の行が載ります。完了条件を削るためではなく、実施できないことを実施したことにしないための節です。
- **報告**: 実装役の `report.md` に `## 未検証項目` が必須になります。節が無い・空の報告は受け取らず、出力が無いのと同じ扱いで人間に戻ります（該当が無ければ「無し」と書けば通ります）。

宣言を持たないプロジェクトの挙動は変わりません（警告も出ず、依頼文の節も増えません）。

既存の票が何件警告になるかは `kb capcheck [--pj P] [--all] [--json]` で数えられます。DB も本文も変更しません。語彙を足すかどうかは、この測定結果を見てから判断してください。
