...
Windowsでは、専用VM内のサービスがLocalSystemで動き、タスクは別の一般ユーザーで実行します。サービスのトークン・ジャーナル・タスクアカウントのパスワードはACLで保護し、タスクから読めないようにします。タスクにはプロジェクト用の認証情報を一時的に渡します。

通信制限はProxmox側で設定します。返却時に削除するのはrunの作業フォルダーと一時プロファイルで、OS全体は巻き戻しません。同じ管理主体の信頼できるプロジェクト向けの専用VMとして使います。停止・成果物回収・復旧の詳細は[Windowsワーカーの導入と運用](../guides/windows-worker.md)を参照してください。

以下の `clean` スナップショットとtmpfsの説明は、従来のLinux VMバックエンドの構成です。
...
制御系を Proxmox 上の LXC に置く構成（ADR-0017）では、「信頼する側」が Mac から制御系 LXC に移ります。制御系は Proxmox に対して **自分のリソースプールだけに効く API トークン**（一覧・起動・巻き戻し）しか持たず、ホストの root を持ちません。貸出先の組織ごとに網（別 vnet と /16）・プール・秘密情報が分かれ、VM も制御系も RFC1918 全体と tailnet 宛てを DROP するので他の組織の環境には届きません。コンソールは合言葉（`CONSOLE_TOKEN`）が無いと 127.0.0.1 の外に出ません。[貸出先ごとの環境（テナント）](../guides/tenants.md)
...
Linuxのワーカートークンとジャーナルはroot専用とし、タスクは別の一般ユーザーで実行する。systemdのcgroupでコマンドの子孫を管理し、停止が未確認なら予約を保持する。返却は作業フォルダーの削除で、OSやGUIの状態は戻さない。ネットワーク制限はインスタンスのファイアウォールで設定する。[範囲と制限](../guides/linux-worker.md)。
