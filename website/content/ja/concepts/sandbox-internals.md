...
貸し出されていないプール VM は、放っておくと `running` のまま CPU とメモリを占有します。制御系の systemd timer が 15 分ごとに `sandbox idle-stop` を呼び、**貸出中でなく、最後に使われてから 3 時間（既定、設定可）経った**プール VM を止めます（ADR-0033）。
