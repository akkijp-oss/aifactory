...
このページで分かること: `sandbox take` の 1 コマンドで何が起こるか、VM のプールと巻き戻しの仕組み、ネットワーク、命名の規則。Proxmox 側の詳細は `docs/sandbox-architecture.html` と `sandbox/README.md`。

## 5 操作の契約

sandbox は `sandbox` CLI の 5 操作で完結します。これが区画間のインターフェースで、Proxmox の都合はこの内側に閉じ込めます。
...
出力物（diff、PR、テスト結果）は sandbox の責務ではありません。VM の中で workflow が作り、`ssh` か `git push` で外に出します。
...
| 段階 | 何を | 所要 |
|---|---|---|
| 空き VM の選択 | `~/.config/sandbox/state.json` の貸出台帳から、その PJ のプールで空いている VM | 即 |
| 巻き戻し | Proxmox で `clean` スナップショットに `qm rollback`。RAM 込みなので起動待ちが無い | 数秒 |
| トークン注入 | PJ の Claude トークンと、GitHub App の installation token（そのリポジトリ限定・1 時間）を `/run/sandbox/env` に。tmpfs なので巻き戻しで消える | 1〜2 秒 |
...
- `clean` は「アプリが :3000 で起動済み」「firewall 設定込み」の状態で取る。だから take 直後にブラウザで開ける
- テンプレートは 2 層。base（全 PJ 共通）と PJ 層（Ruby / Node の版、依存、seed）。Ruby / Node の版が PJ で違うのでテンプレートは PJ 単位
...
| 通信制限 | VM 発の NEW 接続は インターネットと sb-gw の DNS だけ。LAN・ホスト・隣 VM・tailnet は DROP（ADR-0010） | 隣の VM やホストの SSH に届くのを塞ぐ |
| CI | 新設せず、PJ 側の既存の CI（GitHub Actions）に任せる | 既存資産 |

## 命名と採番

| 対象 | 規則 | 例 |
|---|---|---|
| VMID | 9000 = ゲートウェイ（`SB_GW_CT`）、9100 = base（`SB_BASE_VMID`）、911x = PJ テンプレート、92xx = プール（`SB_POOL_BASE`） | 9110 = kumitate テンプレ、9204〜9206 = kumitate プール |
| VM 名 | `sb-gw` / `sb-base` / `sb-tpl-{pj}` / `sb-{pj}-{NN}` | `sb-kumitate-01` |
| IP | プール `SB_POOL_NET.(VMID−SB_POOL_BASE)`、既定 `10.77.1.(VMID−9200)`。PJ 横断で一意（ADR-0007） | 9204 → 10.77.1.4 |
| DNS | `task-{id}.sb.internal`（貸出中のみ）、`sb-{pj}-{NN}.sb.internal`（常設） | `task-204.sb.internal` |
| スナップショット | `clean` | |
| VM 内ユーザー | `dev`（sudo 可、パスワード無し） | |
| サイズ | 汎用 4 GB / 2 vCPU、PJ プール 8 GB / 4 vCPU | |
...
| `~/work/<id>/` | artifact 置き場。`ticket.md` → `plan.md` → `report.md` … 。release 時に Mac へ回収 |
...
- 20 台規模のプール（PJ × 3 + 汎用 3）でも linked clone なので `local-lvm` の実使用は数 %
- 契約の 5 操作は workflow から見て足りた。足したのは `reinject`（トークン更新）だけ
