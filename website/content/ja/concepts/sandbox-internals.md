...
| 操作 | 保証すること |
|---|---|
| `take <pj> <task-id>` | クリーンな VM を 1 台確保し、`task-<id>.sb.internal` で到達できる状態にする。VM 内に task-id と認証トークンを注入する。VM が節電で止まっていれば起動して ssh を待つ。空きがなければ非 0 で終了 |
...
### 使われていない VM は止める（idle-stop）

貸し出されていないプール VM は、放っておくと `running` のまま CPU とメモリを占有します。制御系の systemd timer が 15 分ごとに `sandbox idle-stop` を呼び、**貸出中でなく、最後に使われてから 24 時間（既定、設定可）経った**プール VM を停止候補にし、**候補が 10 台（既定、設定可）を超えたぶんだけ古い順に**止めます（ADR-0033 / ADR-0035）。

- 起こす仕組みは足していません。`rollback()`（`take` / `reset` / `release` が必ず通る）が「停止中なら起動して ssh を待つ」を元から持っているので、次の `take` がそのまま起こします。そのぶん 30〜60 秒ほど余分にかかり、`[start] vm <vmid>: 停止中だったので起動した（N 秒）` がログに出ます
- 最終利用は `last-used.json` に別ファイルで持ちます（貸出台帳 `state.json` は返却で項目が消えるので、返却後の経過時間が分からない）。記録が無い VM は `uptime` で代用し、それも取れなければ止めません
- 判定から停止の完了までは `state.json.lock` を 1 台ずつ持ちます。ロックを先に手放すと、その隙に `take` が同じ VM を予約し、まだ `running` なので起動待ちを飛ばし、直後に電源が落ちて run が死ぬためです
- 制御系の `ctl` / `gw` は LXC なので対象になりません（プールの一覧は qemu 限定）
