...
このページで分かること: 工場の「今どうなっているか」と「何が起きたか」がどこに残り、誰が読むか。会話の記憶に頼らず、次のセッションが引き継げるようにするための置き場です。
...
| run の進行（今の step、ループ回数、結果、PR URL、所要秒） | `workspace/runs/<日付>-<pj>-<id>/state.json` | runner（`--resume`）、`kb sync` | workspace |
| agent が見た依頼文 | 同 `prompt-<step>-<n>.md` | 人間（失敗の切り分け） | workspace |
| agent の出力（人が読める形、逐次） | 同 `agent-<step>-<n>.log` | 人間、Web コンソール | workspace |
| agent の生イベント（stream-json） | 同 `agent-<step>-<n>.jsonl` | デバッグする人 | workspace |
| 今動いている step とログ名 | `state.json` の `current` | Web コンソール、`tail -f` する人 | workspace（終わると null） |
| コンソールが起動した CLI の出力 | `console/jobs/<id>/log`（`CONSOLE_JOBS` で変更可） | Web コンソール | 外 |
| code step の出力 | 同 `code-<step>-<n>.log` | 人間 | workspace |
| artifact（plan.md / report.md / review.md / summary.md / gates.txt） | VM `~/work/<id>/` → release 時に同 `work/` | 次の step、reviewer、人間 | workspace |
| VM の貸出 | `~/.config/sandbox/state.json`（`sandbox ls`） | sandbox CLI、dispatch、runner、他セッション | 外 |
| 起票と配車のログ | `workspace/logs/intake.log` / `workspace/logs/dispatch.log` | 人間 | workspace |
...
| 依頼文 / ログ | `prompt-<step>-<n>.md` / `agent-<step>-<n>.log` / `code-<step>-<n>.log`（n は step の通し番号） | `prompt-review-2.md` |
...
3. `sandbox/STATUS.md` の「実機確認」コマンドを流して実機と照合
