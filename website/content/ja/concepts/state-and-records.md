...
記録に書く日時は、オフセット付きの ISO 8601（`2026-09-08T00:21:00+00:00`）に揃えます。読む側が時間帯を推し量らずに済ませるためです。オフセットの無い古い記録は、書いたホストのローカル時刻とみなして読みます（Web コンソールが読むときに補い、記録そのものは書き換えません）。判断の記録は ADR-0026（[設計判断](../decisions/index.md)）です。
