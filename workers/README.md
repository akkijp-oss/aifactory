...
以下は引数のリファレンス。`uncertain` になった操作を戻す手順（状態の見方 → ゲストの生存確認 → `resolve` → lease解放 → 再開）は [docs/macos-worker.md の「uncertainからの復旧」](../docs/macos-worker.md#uncertainからの復旧)にまとめてある。
...
- ログは1操作16MiBまで。上限に達しても操作は最後まで走り、結果はexit codeで決まる。超過分は捨て、末尾に切り捨て行が入り、結果に `truncated: true` が付く。上限はゲスト停止の理由にならない。永続化エラーだけは通常成功にしない。
- stream-jsonに載る画像のbase64は記録前に `[image N bytes]` へ置き換える。画像本体はゲストの成果物（`screenshot-latest.png` など）で確認する。
