...
- `related_issue` / `related_issue_access`（`--issue` / `--issue-access`）と `related_ticket`（`--ticket`）は**参照の行き先を構造で分ける**列。外部 issue の URL と aifactory の内部票番号を互いの欄に書けない（内部番号を `--issue` に書くと断り、`--ticket` を案内する）。取得可否は `readable` / `unreadable` の 2 語だけで、**書かなければ `unreadable`**（sandbox のトークンは issues に 403 を返すので、確かめていない参照は取りに行かせない）。本文の自由文は解釈せず、既存票の移行もしない（新しい起票から構造化する）。`--issue ''` は URL と取得可否を一緒に消す。存在検査はしない（ADR-0084）。`kb run` はこの 3 列を runner に渡し、依頼文の「## チケット」の直後に値が 1〜2 行出る（列が空の票では出ない。規則は役割文書のまま。チケット 594）
- `base_sha`（`--base-sha`）は**起票した時点の base**（PJ の `base_branch` の tip）の commit sha。票本文が引用する `path:line` が「いつ時点のものか」を機械が持つための列で、console の config 編集が使う `base_sha256`（版ハッシュ）とは別物。人が書くのではなく `kb new` が `gh api repos/<repo>/commits/<base_branch>` で自動で刻む（`--base-sha SHA` で明示すれば GitHub には聞きに行かず、`--base-sha ''` で消せる）。repo / base_branch が無い・トークンが取れない・`gh` が失敗するなどで取れなければ**刻まないだけ**で起票は落とさない（stderr に `[kb] warn:` を 1 行。`[kb] warning:` は console が能力照合の当たりとして拾う印なので分けてある）。`kb run` はこの列を runner に渡し、runner が VM の中で `git rev-list --count <base_sha>..origin/<base>` を測って、進んでいれば依頼文の「## チケット」の**直下**に「何 commits 前の行番号か」を 1 行出す（距離 0 の票と `base_sha` を持たない票では 1 行も出ない）。ADR-0091 / チケット 554
