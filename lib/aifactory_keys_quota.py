...
"""
import argparse, datetime, json, os, pathlib, sqlite3, sys, time, urllib.error, urllib.request
...
def probe_key(token, model, *, base_url=DEFAULT_BASE_URL, timeout=20, now=None, request=_request):
    ...
    snap = {"probed_ts": ts, "model": model, "http_status": None, "error": None, "windows": {}, "covers_all": False,
            "status": None, "claim": None, "overage_status": None, "overage_reason": None}
    try:
...
class Store:
    ...
    def forget(self, name):
        ...
        with self.c:
            self.c.execute("DELETE FROM key_quota WHERE name=?", (name,))
            self.c.execute("DELETE FROM key_quota_history WHERE name=?", (name,))

    def keep_only(self, names):
        names = list(names)
        with self.c:
            q = ",".join("?" * len(names)) or "''"
            self.c.execute(f"DELETE FROM key_quota WHERE name NOT IN ({q})", names)
            self.c.execute(f"DELETE FROM key_quota_history WHERE name NOT IN ({q})", names)

    def record(self, name, tail4, snap):
        ...
        with self.c:
            ...
            self.c.execute("UPDATE key_quota SET tail4=?, probed_ts=?, model=?, http_status=?, error=?, updated_ts=? WHERE name=?",
                           (tail4, ts, snap["model"], snap["http_status"], snap["error"], now_ts(), name))
            if not ok: return
    ...
    def prune(self, keep_days=DEFAULT_KEEP_DAYS):
        with self.c:
            return self.c.execute("DELETE FROM key_quota_history WHERE probed_ts < ?", (now_ts() - keep_days * 86400,)).rowcount or 0
...
def run_probe(keys_file=None, db_file=None, *, mode="auto", env=None, probe=None, now=None):
    ...
    st = Store(db_file or quota_db_path(kp, env))
    keys = load_keys(kp)
    ts = now if now is not None else now_ts()
    out = {"keys_file": str(kp), "db_file": str(st.path), "probed": [], "skipped": [], "pruned": 0}
    try:
        ...
        for k in keys:
            ...
            prev = st.row(name)
            if prev and prev.get("tail4") and prev["tail4"] != tail4: st.forget(name)   # 値が入れ替わった → 別の契約かもしれない
...
def window_summary(util, reset_ts, status, key, now):
    ...
    exhausted = util >= 0.99 or status == "rejected"
    remaining_pct = 0.0 if exhausted else round((1.0 - util) * 100, 1)
...
def main(argv=None):
    ...
    if a.cmd == "probe":
        r = run_probe(mode="full" if a.full else "cheap" if a.cheap else "auto")
