...
- 「鍵」画面の上には鍵ごとの**残量（利用枠）**（5 時間枠 / 7 日枠（全体）/ 7 日枠（Fable）の残り %・リセットまでの時間・枯渇予測）が出ます。制御系の timer `aifactory-keys-probe.timer`（`install.sh --systemd` で登録。5 分ごと）が `lib/aifactory_keys_quota.py probe` で観測し、`keys.json` の隣の `keys-quota.db` に記録します。手で見るなら `python3 lib/aifactory_keys_quota.py show`（ADR-0087）。
