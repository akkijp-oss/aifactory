...
- 「鍵」画面の上には鍵ごとの**残量（利用枠）**（5 時間枠 / 7 日枠（全体）/ 7 日枠（Fable）の残り %・リセットまでの時間・枯渇予測）が出ます。制御系の timer `aifactory-keys-probe.timer`（`install.sh --systemd` で登録。5 分ごと）が `lib/aifactory_keys_quota.py probe` で観測し、`keys.json` の隣の `keys-quota.db` に記録します。手で見るなら `python3 lib/aifactory_keys_quota.py show`（ADR-0085）。
...
| コミットの author date や ADR の日付が数日前になる / run が `failure: clock` で止まる | run の記録の `clock_offset_s`、`sandbox take` のログの `[clock]` 行、VM の中で `timedatectl` | 巻き戻した VM は snapshot 時刻から時計が再開する（ADR-0069）。`failure: clock` で止まった run は VM を返しているので `kb reopen` → `kb run` で回し直すだけ（次の take が合わせる）。貸出中のまま日付がずれているなら `sandbox reset <task>` で貸し直すと take が合わせる。合わないなら VM の中で `sudo systemctl restart systemd-timesyncd`、それでも駄目なら NTP（udp/123）が出られるかを見る。許容は `SB_CLOCK_TOLERANCE_S` / `AIFACTORY_CLOCK_TOLERANCE_S`（既定 120 秒） |
