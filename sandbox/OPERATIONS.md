...
| コミットの author date や ADR の日付が数日前になる / run が `failure: clock` で止まる | run の記録の `clock_offset_s`、`sandbox take` のログの `[clock]` 行、VM の中で `timedatectl` | 巻き戻した VM は snapshot 時刻から時計が再開する（ADR-0069）。`sandbox reset <task>` で貸し直すと take が合わせる。合わないなら VM の中で `sudo systemctl restart systemd-timesyncd`、それでも駄目なら NTP（udp/123）が出られるかを見る。許容は `SB_CLOCK_TOLERANCE_S` / `AIFACTORY_CLOCK_TOLERANCE_S`（既定 120 秒） |
