...
- [ ] 🧑 貸出先（または自分）が LXC の中で secrets を入れる: `sandbox token set <pj>`、`sandbox/bin/gh-app-setup`（ブラウザ操作は手元で）、`~/.config/aifactory/ctl.env` の `CLAUDE_CODE_OAUTH_TOKEN`（intake 用）。`GH_TOKEN` は GitHub App があれば空でよい（runner が `sandbox gh-app token <pj>` で払い出す。ADR-0029）
