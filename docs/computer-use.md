...
MCPの画面取得は作業フォルダーの `screenshot-latest.png` を更新する。最後の1枚を通常の成果物としてSHA-256確認後に回収する。複数枚必要なら、エージェントが作業フォルダー直下に別名の通常ファイルとして保存する。サブディレクトリに置いた画像は回収されない。全成果物の合計上限は4 MiB。画面には機密情報が写り得るため、成果物の閲覧者にも同じ情報が見えることに注意する。
...
`key` に渡せる名前は3 OSで同じで、次の79個。大文字小文字は区別しない。

| 種類 | 名前 |
| --- | --- |
| 修飾キー | `CTRL` `ALT` `SHIFT` `WIN` `CMD` |
| 特殊キー | `ENTER` `TAB` `ESC` `SPACE` `BACKSPACE` `DELETE` `LEFT` `RIGHT` `UP` `DOWN` `HOME` `END` `PAGEUP` `PAGEDOWN` |
| ファンクション | `F1`〜`F12` |
| 英数字 | `A`〜`Z` / `0`〜`9` |
| 記号 | `=` `-` `+` `,` `.` `/` `;` `'` `[` `]` `\` `` ` `` |
...
キー操作は「ウィンドウにフォーカスがあり、そのアプリがそのショートカットを持っている」ことが前提で、送れたことと効いたことは別。`key` を2回以上繰り返しても表示が変わらないときは、同じ操作を続けずに次へ切り替える。

1. `screenshot` で今の状態を確かめる。フォーカスが別のウィンドウにあるなら、目的のウィンドウをクリックしてから送り直す。
2. メニューをクリックする。表示倍率・保存・検索などはたいていメニューにあり、クリックの方が結果を画面で確認できる。
...
- `scale` は未対応。`tart set` の該当オプションを実機で確認してから別チケットで扱う（[ADR-0057](adr/0057-guest-display-resolution.md)）。
...
記号・ファンクション・特殊キーの対応（チケット344）は、Mac / Windowsの実機では未確認。3 OSでキー名の表が一致していることの自動テストと、CIでのswiftcビルド・PowerShellの構文検査までを確認した。Linuxのみ、Xvfb上のテキスト入力欄へ記号キーを送って本文の一致を確認している。
