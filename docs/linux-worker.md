...
PATHの決め方はmacOSワーカーと同じ規則で、`workflow/lib/macos.py` の `guest_path_prelude()` を共有する。**ゲスト自身に答えさせる**形で、runnerは道具を列挙しない（[ADR-0076](adr/0076-guest-path-is-answered-by-the-guest.md)）。
