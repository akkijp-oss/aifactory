...
2026-09-07 / 状態: ③を採用。VMライフサイクルとworkflow統合を実装し、文書整備からPR作成・VM返却まで実機確認済み

現在の構成・導入・復旧手順は [Macワーカーの導入と運用](macos-worker.md)、実装の契約は [workers/README.md](../workers/README.md) を参照。Mac側はユーザーの希望によりGo製の単一バイナリに変更した。以下は実装着手前の調査記録であり、「未導入」「未検証」や変更候補の記述は調査当時の状態を表す。実装済みのworkflow統合は [ADR-0020](adr/0020-macos-workflow-backend.md) に記録した。
