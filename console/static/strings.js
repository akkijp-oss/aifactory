...
const T = {
  ...
  "btn": {
    ...
    "keyAdd": "この鍵を登録する", "keyRemove": "削除する", "keyToken": "トークンを入れ替える", "keyProbe": "いま調べる",
    "modelPreview": "変更を確かめる", "modelInherit": "継承へ戻す", "blockPreview": "この定義の変更を確かめる",
    "pmMode": "モードを切り替える", "pmPause": "一時停止する", "pmNow": "今すぐ回す",
  ...
  "h": {
    ...
    "keys": "登録してある鍵", "keyAdd": "鍵を登録する", "quota": "残量（利用枠）", "quotaHistory": "残量の推移と予測",
  ...
  "th": {
    ...
    "at": "日時", "pmTarget": "対象", "pmAction": "決めたこと", "field": "項目", "before": "前", "after": "後", "state": "状態", "what": "内容", "rc": "終了コード",
    ...
    "issued": "登録日", "lastLaunched": "最後に起動した日時", "launches": "起動回数", "keyInUse": "使用中のチケット", "quota": "残量"
  },
  ...
  "keys": { "count": "{n} 本", "never": "まだ起動していません", "assigned": "割り当てた回数: {n}（チケットの実行に鍵を渡した回数。起動回数とは別です）",
    "window": { "5h": "5 時間枠", "7d": "7 日枠（全体）", "7d_oi": "7 日枠（Fable）" },
    ...
    "oiPending": "Fable の残量はまだ取得できていません。「いま調べる」で確認できます。",
    "dbError": "残量の記録を読めませんでした（{e}）。",
    "hours24": "前後 24 時間", "days7": "過去 7 日・未来 7 日",
    "oldValue": "前回の値（15 分以上前）", "retry": "読み直す", "selectKey": "残量を見る鍵", "aboutQuota": "利用枠と観測について",
    "now": "現在", "observed": "実測", "predicted": "予測", "lineKey": "実線: 実測 / 破線: 予測",
    "chartRange": "左: 過去 / 右: 未来", "chartKeyboard": "グラフをなぞると残量を表示します。左右キーで 1 時間ずつ移動できます。横に収まらないときはスクロールできます。",
    "fableError": "Fable の残量を取得できませんでした（{e}）。Fable の値は前回取得分です。",
    "forecastReason": { "missing": "観測がなく予測できません。", "stale": "観測が古いため予測を止めています。", "reset": "回復時刻を確認するため、次の観測を待っています。", "early": "枠が始まったばかりのため、予測に必要な観測を待っています。" } },
  ...
  "pm": {
    "loading": "状態を取得しています。",
    "unknownState": "状態を取得できていません",
    "stale": "最後に読めたのは {t} です。",
    "h": { "now": "いま", "next": "次に回すもの", "log": "判断の記録", "board": "板の要約", "ops": "操作" },
    "state": {
      "idle": "回すチケットを待っています",
      "waiting": "run が動いています",
      "landing": "着地を見守っています",
    ...
    "boardReason": {
      "no_db": "まだ記録がありません。チケットを起票すると板ができます。",
      "kb_failed": "板を読めませんでした（{e}）。",
      "other": "板を読めていません。"
    },
    "next": {
      "picked_next": "次はこのチケットです。",
      "no_todo": "回すチケットがありません。",
      "run_running": "run が動いているので、終わるまで次は起こしません。",
      ...
      "blocked_by_pause": "一時停止中のチケットしか残っていないので、解除されるまで次は起こしません。",
      "other": "次を決めた理由をこの画面ではまだ読めません。"
    },
    "pause": {
      "nokey": "{keys} の鍵が鍵プールに登録されるまで待っています。",
      "hits": "利用枠切れが {n} 回続いたので、人が利用枠を確かめるまで待っています。",
      "quota": "利用枠が戻る {until} 以降に続きを回します。"
    },
    "reason": {
      "no_todo": "回すチケットがなかったため",
      "picked_next": "次のチケットを選べたため",
      "run_running": "run が動いていたため",
      "landing_observed": "着地を見守っていたため",
      "merged_observed": "PR が取り込まれたのを見たため",
      "requeued": "同じチケットをもう一度回したため",
      "same_gate_fails": "同じゲートが続けて赤かったため",
      "review_retry_limit": "レビューからのやり直しが上限に達したため",
      "release_path": "本番に出る操作にあたるため",
      "risky_diff": "差分に取り返しのつきにくい変更があったため",
      "forbidden_hint": "PJ が禁じている変更にあたるおそれがあるため",
      "proposed": "自動では進めず、人に提案するため",
      "approved": "人が承認したため",
      "skipped_by_steer": "人がこのチケットを飛ばすよう指示したため",
      "paused": "人が一時停止を指示したため",
      "blocked_by_dependency": "先に終わらせるチケットが終わっていなかったため",
      "blocked_by_pause": "一時停止中のチケットしか残っていなかったため",
    ...
    "blockedHead": "人の確認を待っているチケット: {n} 件",
    "blockedPj": "この一覧は PJ {pj} の分です。",
    "blockedUnknown": "人の確認を待っているチケットの件数を読めていません。",
    "blockedMore": "ほか {n} 件はボードの「人間待ち」で見られます。"
  },
  ...
  "outcome": {
    ...
    "clock_skew": "ゲストの時計が制御系と {n} 秒ずれていたので、工程を 1 つも始めずに止めました（そのまま回すとコミットと ADR の日付が嘘になります）。VM は返してあるので、そのまま回し直してください（次の貸出で時計を合わせ直します）。",
  ...
  "config": {
    ...
    "modelAgent": "Agent", "modelName": "モデル名", "modelOther": "その他（直接入力）",
  ...
  "empty": {
    ...
    "quota": "まだ観測していません。「いま調べる」を押すと、その場で観測します。",
    "quotaChart": "まだ履歴がありません。観測を重ねると線が描かれます。",
  ...
  "help": {
    ...
    "configModelPick": "Agent とモデル名を選ぶと、下の欄にモデルの ID が入ります。表にないモデルは、下の欄に直接入力できます。",
    ...
    "quota": "鍵の利用枠は 3 つの窓に分かれています。5 時間枠はすべてのモデルで共通です。7 日枠（全体）は 1 週間の枠です。7 日枠（Fable）は Fable だけが使う 1 週間の枠です。",
    "quotaHow": "使わない設定の鍵も含め、すべての鍵に 5 分ごとに Fable で問い合わせます。共通の枠を取得できないときは、別のモデルでも確認します。問い合わせでは少量の利用枠を消費します。",
    "quotaTimer": "定期的な観測は制御系の timer（aifactory-keys-probe.timer）が行います。「いま調べる」で、その場でも観測できます。",
    "quotaChart": "予測は、現在の枠が始まってからの平均消費ペースが続く場合の目安です。回復時刻で 100% に戻り、その後も同じ間隔で回復すると仮定しています。実際の利用や回復時刻により変わります。実測のない期間は線をつなぎません。",
    ...
    "pmOps": "これらの操作はまだつながっていません。押しても何も起こりません。あとの更新で使えるようになります。",
    "pmAllPj": "すべての PJ をまとめて見ています。どれか 1 つでも動いていれば「run が動いています」と出ます。PJ を選ぶと、その PJ だけで見直します。",
    "pmCountsAll": "この件数はすべての PJ の合計です。左のナビの数字と同じで、上で PJ を選んでも変わりません。",
    "pmReadOnly": "この画面は記録を読んで並べるだけです。開いても、更新しても、run は起きません。"
  },
  ...
  "msg": {
    ...
    "keyProbeStarted": "残量の観測を始めました。数十秒で更新されます。", "keyProbeRunning": "残量の観測はすでに動いています。",
  ...
  "dialog": {
    ...
    "dispatch": {
      ...
      "unchecked": "VM の空き・worker・project.yml は、配車するときに確かめます。",
      "skippedDep": "{id} は先に終わらせるチケット（{deps}）が終わっていないので飛ばします。",
      "skippedPause": "{id} は一時停止中（{until} 以降に続きを回します）なので飛ばします。",
      "skippedNoKey": "{id} は鍵の登録待ちで一時停止中なので飛ばします。",
      "skippedHits": "{id} は鍵の利用枠切れが {n} 回続いているので、自動では回しません。",
      "unreadable": "次に回るチケットを確かめられませんでした。配車すると何が回るかは、出力で確かめてください。",
