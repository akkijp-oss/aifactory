...
const T = {
  ...
  "sandbox": {
    ...
    "keySource": { "pool": "鍵プール", "pool_fable_only": "鍵プール（Fable 用だけ）", "pool_other_only": "鍵プール（Opus・Sonnet 用だけ）", "none": "未登録（run は一時停止）" },
  ...
  "help": {
    ...
    "pjPoolKeys": "Claude の鍵は「鍵」画面のプールからだけ選ばれます（PJ ごとの設定ファイルには置けません）。Fable 用と Opus・Sonnet 用の両方がそろっていない用途の工程がある run は、VM を借りずに一時停止します。値は見ていません（有無だけ）。",
    ...
    "keysFallback": "VM に渡る Claude の鍵はこの一覧からだけ選ばれます。合う鍵が 1 本もない用途の工程がある run は、VM を借りずに一時停止し、ここに登録すると自動で回し直します。",
