...
- `--from[=step]` は human で止まった run を**新しい VM**で指定の step からやり直す（`--branch=<名前>` の続きから。既定は前回の `wip_branch`）。step を省くと前回の `resume_step` を使う。前回の run は `AIFACTORY_FROM_RUN`（run 名の形だけ）で渡し、その `work/*.md` を持ち込み、`review.md` を最初の依頼文に「前回の結果（直すこと）」として入れる（ADR-0034）
