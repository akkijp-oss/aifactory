...
### Added
- **runner: 掃き寄せが起きた事実とファイル一覧が記録に残る**。掃き寄せコミットの本文に拾ったファイルと除外して戻したファイルの一覧が入り（今までは本文ゼロ）、`state.json` の工程履歴の `swept` とコンソール / MCP の `outcome.swept`、runner の標準出力にも出る。差分を開かないと気づけない状態をやめた
